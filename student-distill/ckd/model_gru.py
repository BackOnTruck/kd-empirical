...
from loss import ckd_loss

from params import *

HIDDEN, N_LAYERS = (176, 2) if args.tokenizer == '2000' else (320, 3) if args.tokenizer == '5000' else (448, 6)
...
class Student(RobertaPreTrainedModel):
    def __init__(self, config: RC, model: RSC):
        config.hidden_size, config.num_hidden_layers = HIDDEN, N_LAYERS
        ...
        for i in range(s_layer):
            self.add_module(MultiLayered([RL(large_cfg) for _ in range(size)]),
                ...
                nn.GRU(input_size=HIDDEN if i == 0 else 2 * HIDDEN, hidden_size=HIDDEN, batch_first=True, bidirectional=True))
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor, token_type_ids: Tensor,
        ...
        feat, t_feat = [], [] # [n_layers, batch_size, seq_length, hidden_size]

        for (mod, t_mod) in zip(self.mods[1:-1], self.t_mods[1:-1]):
            res = mod(res)[0]
            t_res = t_mod(t_res, t_attn_mask)[0] if mode else None
            feat += [res]
            t_feat += [t_res if mode else None]

        res = self.mods[-1](torch.cat((res[:, -1, :HIDDEN], res[:, 0, HIDDEN:]), dim=1).unsqueeze(1))
        t_res = self.t_mods[-1](t_res) if mode else None

        feat = torch.stack(feat).transpose(0, 1).contiguous() # [batch_size, n_layers, seq_length, hidden_size]
        t_feat = torch.stack(t_feat).transpose(0, 1).contiguous() if mode else None

        if mode: return res, ce(res / TEMP, F.softmax(t_res / TEMP, dim=-1)) * TEMP ** 2 * ALPHA + ce(res, labels) * (1 - ALPHA) \
            + ckd_loss(feat, t_feat, WINDOW) * BETA
        return res, None
...
class SavedModel(RobertaPreTrainedModel):
    def __init__(self, config: RC):
        ...
        self.layers = nn.GRU(input_size=HIDDEN, hidden_size=HIDDEN, num_layers=N_LAYERS, bidirectional=True, batch_first=True)
    ...
    def load_student(config: RC, model: Student):
        ...
        mod.layers.load_state_dict({name.replace('0', str(i)): params
            for i, layer in enumerate(model.mods[1:-1])
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor, token_type_ids: Tensor, *args):
        ...
        return self.classifier(torch.cat((res[:, -1, :HIDDEN], res[:, 0, HIDDEN:]), dim=1).unsqueeze(1)), None
