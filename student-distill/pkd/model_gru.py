...
class MultiLayered(nn.Module):
    ...
    def forward(self, *args):
        res, others, all_hidden = args[0], args[1:], []

        for trans in self.mods:
            res = trans(res, *others)[0]
            all_hidden += [res]

        return [res, torch.stack(all_hidden)]
...
class Student(RobertaPreTrainedModel):
    def __init__(self, config: RC, model: RSC):
        ...
        self.interm = nn.ModuleList([nn.Linear(t_hidden, s_hidden * 2) for _ in range(s_layer)])
        self.post_init()
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor, token_type_ids: Tensor,
        ...
        res, feat = self.mods[1](res) # feat: (2 * layers, batch size, hidden)
        t_res, t_feat = self.t_mods[1](t_res, t_attn_mask) if mode else (None, None) # t_feat: (layers, batch size, length, hidden)

        res = self.mods[-1](torch.cat((res[:, -1, :RNN_HIDDEN], res[:, 0, RNN_HIDDEN:]), dim=1).unsqueeze(1))
        t_res = self.t_mods[-1](t_res) if mode else None

        # for PKD loss: transform both feature tensors into shape (layers, batch size, hidden * 2)
        feat = torch.stack([torch.cat((feat[2 * i], feat[2 * i + 1]), dim=-1) for i in range(feat.size()[0] // 2)])[:-1]
        t_feat = torch.stack([self.interm[layer](t_feat[(layer + 1) * self.size - 1, :, 0, :]) for layer in range(feat.size()[0])]) if mode else None
